...
int main() {
    ...
    while (deck.size() != 0 && p1.getHandSize() != 0 && p2.getHandSize()) {             //even if all cards are dealt, there might be cards in players hands to ask for
        while (successfulFish) {
            ...
            if (p2.cardInHand(c1)) {
                ...
                p1.bookCards(c1, c2);
                cout << p1.getName() << " books his pair of " << c1.rankString(c1.getRank()) << "'s. (" << p1.getBookSize()/2 << ")" << endl << endl;
            } else {
                ...
                if(deck.size() != 0) {
                    ...
                    if (p1.checkHandForBook(c1, c2)) {
                        ...
                        cout << "From the card he drew, " << p1.getName() << " books a pair of "
                             << c1.rankString(c1.getRank()) << "'s. (" << p1.getBookSize() / 2 << ")" << endl << endl;
                    }
        ...
        while (successfulFish) {
            ...
            if (p1.cardInHand(c1)) {
                ...
                p2.bookCards(c1, c2);
                cout << p2.getName() << " books his pair of " << c1.rankString(c1.getRank()) << "'s. (" << p2.getBookSize()/2 << ")" << endl << endl;
            } else {
                successfulFish = false;
                ...
                if (deck.size() != 0) {
                    ...
                    if (p2.checkHandForBook(c1, c2)) {
                        ...
                        cout << "From the card he drew, " << p2.getName() << " books a pair of "
                             << c1.rankString(c1.getRank()) << "'s. (" << p2.getBookSize() / 2 << ")" << endl << endl;
                    }
                }else cout << "The deck is empty, so no card is drawn." << endl << endl;
            }
        }
